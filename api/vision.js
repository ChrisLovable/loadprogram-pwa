...
app.post('/analyze', upload.single('image'), async (req, res) => {
  try {
    ...
    const prompt = `You are an expert at reading scanned delivery documents. Focus ONLY on the very top left of the document. Ignore all other text. For both 'SENDER:' and 'RECEIVER:', extract ALL consecutive handwritten lines after each label (not just the first line), stopping at the next label, empty line, or unrelated field. Join these lines with commas. Return as JSON: { sender: ..., receiver: ... }. If you cannot find the label or handwriting, return null for that field. Also, return the raw OCR text as 'raw_text' for debugging. Be robust to faint, slanted, or cursive handwriting. Do not guess, only extract what is clearly present after the labels.`;
